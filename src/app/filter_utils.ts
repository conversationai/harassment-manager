...
export function buildDateFilterForNDays(now: Date, days: number,from:DayFilterType=DayFilterType.MIDNIGHT): DateFilter {

  const fromTime = from === DayFilterType.NOW  ? now.getTime() : new Date(now).setHours(0, 0, 0, 0);
